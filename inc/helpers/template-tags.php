 ...
 * @return void
 */
function got_posted_by() {
	$byline = sprintf(
		esc_html_x( ' by %s', 'post author', 'got' ),
 ...
 * @return bool|string
 */
function got_the_excerpt( $trim_character_count = 0 ) {
...
function got_excerpt_more( $more = '' ) {
	...
	if ( ! is_single() ) {
		$more = sprintf( '<a class="got-read-more text-white" href="%1$s"><button class="mt-3 btn btn-info">%2$s</button></a>',
...
function got_pagination() {
	...
	printf( '<nav class="got-pagination clearfix">%s</nav>', wp_kses( paginate_links( $args ), $allowed_tags ) );
}
