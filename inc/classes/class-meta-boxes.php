...
class Meta_Boxes {
	...
	public function custom_meta_box_html( $post ) {
		...
		<select name="got_hide_title_field" id="got-field" class="postbox">
			<option value=""><?php esc_html_e( 'Select', 'got' ); ?></option>
			<option value="yes" <?php selected( $value, 'yes' ); ?>>
				<?php esc_html_e( 'Yes', 'got' ); ?>
			</option>
			<option value="no" <?php selected( $value, 'no' ); ?>>
				<?php esc_html_e( 'No', 'got' ); ?>
			</option>
		</select>
		<?php
	}
	...
	public function save_post_meta_data( $post_id ) {
		...
		if ( array_key_exists( 'got_hide_title_field', $_POST ) ) {
			update_post_meta(
				$post_id,
				'_hide_page_title',
				$_POST['got_hide_title_field']
			);
		}
	}
